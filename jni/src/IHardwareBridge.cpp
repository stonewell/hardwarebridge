...
namespace hardwarebridge {
	...
	public:
		...
		virtual int get_flashlight_enabled() {
			Parcel data, reply;
			data.writeInterfaceToken(IHardwareBridge::getInterfaceDescriptor());
			remote()->transact(GET_FLASHLIGHT_ENABLED_TRANSACTION, data, &reply);
			return reply.readInt32();
		}
		...
		virtual int set_flashlight_enabled(int on) {
			...
			data.writeInterfaceToken(IHardwareBridge::getInterfaceDescriptor());
			data.writeInt32(on);
			remote()->transact(SET_FLASHLIGHT_ENABLED_TRANSACTION, data, &reply);
			return reply.readInt32();
		}
		...
		virtual int enable_camera_flash(int milliseconds) {
			Parcel data, reply;
			data.writeInterfaceToken(IHardwareBridge::getInterfaceDescriptor());
			data.writeInt32(milliseconds);
			remote()->transact(ENABLE_CAMERA_FLASH_TRANSACTION, data, &reply);
			return reply.readInt32();
		}
	};
